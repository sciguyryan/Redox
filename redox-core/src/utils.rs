...
pub fn is_bit_set(value: u32, bit: u8) -> bool {
    unsafe { (value & BIT_MASKS_U32.get_unchecked(bit as usize)) != 0 }
}

#[inline(always)]
pub fn is_bit_set_u8(value: u8, bit: u8) -> bool {
    unsafe { (value & BIT_MASKS_U8.get_unchecked(bit as usize)) != 0 }
}

#[inline(always)]
pub fn set_bit_state(value: u32, bit: u8, state: bool) -> u32 {
    unsafe {
        let mask = BIT_MASKS_U32.get_unchecked(bit as usize);
